...
//    FDisplayFileName: TStrings;
  ...
  public
    ...
    procedure Sendmail( User, Passwd: String );
    procedure Reset();
...
procedure TMapiControl.Sendmail( User, Passwd: String );
...
begin
  ...
  MStatus := MapiLogon(AppHandle, {$IFDEF Delphi12}PAnsiChar(AnsiString(User)), PAnsiChar(AnsiString(Passwd)){$ELSE}PAnsiChar(User), PAnsiChar(Passwd){$ENDIF}, MAPI_NEW_SESSION, 0, @MAPI_Session);
  if MStatus <> SUCCESS_SUCCESS then exit;
