{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzCheckList          Enhanced list box where each item is associated with a check box.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Fixed problem where ItemIndex was not being maintained in TRzCheckList when an Item's caption
                          was changed.

  3.0.8  (29 Aug 2003)  * Group code was moved from the TRzCheckList class to the TRzCustomListBox.
                        * Fixed CheckGroup, UncheckGroup, EnableGroup, DisableGroup methods.
                        * Added the AddItemToGroup and InsertItemIntoGroup methods. Also added the following support
                          methods: ItemsInGroup and ItemIndexOfGroup.

  3.0.3  (21 Jan 2003)  * Test FullColorSupport before all calls to PaintGradient.

  3.0    (20 Dec 2002)  * Fixed problem where check and enabled states did not get updated correctly when the
                          Items.Exchange method was used.
                        * The TRzCheckList now supports grouping items in the list. By specifying a prefix "//" at the
                          beginning of an item, the check list will display that item as a group header rather than a
                          regular check item.  There are several new methods that allow users to manipulate the items
                          within the group.  For example, SetGroupState, CheckGroup, UncheckGroup, EnableGroup,
                          DisableGroup. It is also possible to convert an item to a group and back using the ItemToGroup
                          and GroupToItem methods.
                        * Added LoadFromFile, LoadFromStream, SaveToFile, and SaveToStream methods, which not only save
                          the text of each item, but also the State of each item (i.e. cbChecked, cbUnchecked, cbGrayed)
                          and the Enabled state of each item.
