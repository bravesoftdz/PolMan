{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzComboBox           Raize Combo Box.  Provides incremental keyboard searching, AutoComplete, and Custom Framing.
  TRzColorComboBox      Drop-down list populated with color values (incl. System Colors)
  TRzFontComboBox       Select fonts from drop-down list.
  TRzMRUComboBox        Component automatically manages Most Recently Used items.
  TRzImageComboBox      Images can be displayed next to each item.  Items can also be indented.

  NOTE:
  wm_NCPaint processing used in other controls to draw the frame does not work on combo boxes. The actual border is
  drawn in the default wm_Paint handler.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Fixed problem where OnSelEndOK was not fired when user selects an item from a TRzComboBox by
                          pressing Enter.

  3.0.8  (29 Aug 2003)  * Fixed problem where AutoDropDown functionality was not working.

  3.0.5  (24 Mar 2003)  * Fixed problem where Alt+F4 would not exit app if focus was in a TRzComboBox with its ReadOnly
                          property set to True.
                        * Fixed problem where it was possible to type in characters into a ReadOnly combo box.

  3.0.4  (04 Mar 2003)  * Made IndexOf virtual. Overrode IndexOf in TRzCustomImageComboBox to correctly search for
                          matching caption.
                        * Fixed problem where items in the list could not be moved or exchanged without raising an
                          exception.
                        * Added Delete method to TRzCustomImageComboBox, which should be used to manually delete an item
                          from the list.  This will cause the associated object to be freed.
                        * Added MruRegIniFile property to TRzMRUComboBox. This property should be used instead of
                          MRUPath.  MRUPath is still provided for backward-compatibility.

  3.0.3  (21 Jan 2003)  * Added IsColorStored and IsFocusColorStored methods so that if control is disabled at design-
                          time the Color and FocusColor properties are not streamed with the disabled color value.

  3.0    (20 Dec 2002)  << TRzCustomComboBox and TRzComboBox >>
                        ...
                        * Fixed problem where an item that was the same as another item except in a different case could
                          not be selected.
                        * Added AutoComplete property.  This will allow a user to disable the auto-complete feature of
                          the combo box.  Also added the ForceText method, which turns off AutoComplete, sets the text
                          and then turns on AutoComplete.
                        * Fixed problem where mouse wheel may not always scroll up to the top of the list.
                        * When the combo box is csDropDownList and the user presses the Escape key while the List is
                          down, the list only closes and does not put the first item in the list into the text area.
                        * ComboBox button style has been updated.
                        * Added the KeepSearchCase property.  This property controls whether or not the case of the
                          search string the user types in is maintained as matches are found.
                        * Fixed problem where moving the mouse over the edit portion of the combo box caused the
                          selected item in the list to be displayed in the edit portion.  This problem occurred when the
                          FrameFlat (now called FrameHotTrack) property was set to True.
                        * Added XP visual style support in drawing of buttons during Custom Framing.

                        << TRzFontCombobox >>
                        * Added a ShowSymbolFonts property.
                        * Added a new ShowStyle called ssFontPreview. When this style is selected, when the user drops
                          the list of fonts down, a preview window is displayed that shows a sample string formatted in
                          the selected font.  This effect is similar to the one used in CorelDRAW when selecting fonts.
                          The size of the preview window can be changed with the PreviewWidth and PreviewHeight
                          properties. The sample text displayed can be changed using the PreviewText property, or by
                          specifying the PreviewEdit property. When the PreviewEdit property is set to a valid
                          TCustomEdit and there the user has selected some text in the edit control, the selected text
                          is used as the preview text.
                        * The TRzFontComboBox now maintains the most recently used fonts selected by the user.  As the
                          user selects a font, the font is added to the top of the list.  If the font is already in the
                          MRU section, then it is moved to the top of the list when selected.

                        << TRzColorComboBox >>
                        * The LoadCustomColors and SaveCustomColors methods have been changed. Instead of passing the
                          path to the registry key of where to store the custom colors, the TRzColorComboBox needs to be
                          linked to a TRzRegIniFile component to handle saving the information off to either an INI file
                          or the Registry.
