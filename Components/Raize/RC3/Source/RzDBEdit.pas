{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Added OnGetWeekNumber event to TRzDBDateTimeEdit. Handle this event to implement a customized
                        ...
                        * Added OnRangeError event to TRzNumericEdit. This event is generated when the user leaves the
                          ...
                          properties.
                        * Fixed problem where Line property in TRzDBMemo and TRzDBRichEdit would not return the correct
                          value when text was selected.

  3.0.8  (29 Aug 2003)  * Fixed problem where user could not enter a positive number into a TRzDBNumericEdit if the
                          first character in the selection was a '-' sign.
                        * Fixed problem where taking a screen capture of a TRzDBEdit when PasswordChar was <> #0
                          caused the real text to appear in the capture.
                        * Fixed display problems when edit controls were placed on a TDBCtrlGrid.
                        * Added OnDateTimeChange to TRzDBDateTimeEdit. This event fires whenever the date/time value in
                          the edit field changes.
                        * Fixed problem where selected text at beginning of a TRzDBNumericEdit that contained a '+' or
                          '-' sign could not be replaced by pressing the '+' or '-' keys.

  3.0.6  (11 Apr 2003)  * Fixed problem where using the TimePicker to set the time in a TRzDBDateTimeEdit's Time to
                          12:00 AM did not enter 12:00 AM into the edit field.
                        * Fixed problem where a digit could be entered to the left of a sign symbol in TRzDBNumericEdit.
                        * Fixed problem where drop-down TimePicker was closing in response to the wrong event.

  3.0.5  (24 Mar 2003)  * Fixed problem with enter 12:00 am into TRzDBDateTimeEdit and leaving the field changed the
                          time to 12:00 PM.
                        * Clear method now correctly clears the TRzDBDateTimeEdit and it internal datetime value and
                          sets the corresponding database field to NULL.
                        ...
                        * Fixed problem in TRzDBEdit.WMPaint where Msg.DC was being passed to SendMessage as a
                          WParam and was not casted to a WParam (i.e. Longint).
                        * Fixed problem where leaving a TRzDBDateTimeEdit (with EditType=etTime) after deleting selected
                          time value caused 12:00 AM (or appropriately formatted time) to appear in control.
                        * Fixed problem where clearing the date/time value from a TRzDBDateTimeEdit did not write NULL
                          to the database field.

  3.0.4  (04 Mar 2003)  * Fixed problem where empty TRzDBNumericEdit would not write NULL to database field even if
                          AllowBlank was set to True.
                        * Fixed problem where format of TRzDBDateTimeEdit would change when control received the focus.
                        * Fixed problem where OnExit event for TRzDBEdit was getting fired twice.
                        * Fixed problem where focus would leave TRzDBDateTimeEdit if accelerator pressed and the key
  ...
  3.0.3  (21 Jan 2003)  * Added IsColorStored and IsFocusColorStored methods so that if control is disabled at design-time
                        ...
                        * Added additional hot keys to TRzDBDateTimeEdit for changing Day, Month, Year, and Hour, Minute.

  3.0    (20 Dec 2002)  * TRzDBEdit now descends from TRzCustomEdit instead of from TDBEdit. This change was needed
                          so that we can have more control over the painting of the data-aware version. The TDBEdit
                          component handles the wm_Paint message and does its own painting of the control.
...
{$R-}                         // Range checking must be turned off so that TRzDBEdit can operate correctly in DBCtrlGrid
