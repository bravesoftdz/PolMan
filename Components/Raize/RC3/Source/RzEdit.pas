{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzCustomEdit         Base class for all Raize edit-style components--supports Custom Framing and true text alignment.
  TRzEdit               Published version of TRzCustomEdit.
  TRzNumericEdit        Edit control for entering numeric data (integers and floating-point numbers).
  TRzMaskEdit           TMaskEdit descendant--adds Custom Framing support.
  TRzExpandEdit         Edit field can expand to a larger size on enter, or via mouse click.
  TRzColorEdit          Edit field that allows user to pick a color from a popup TRzColorPicker.
  TRzDateTimeEdit       Edit field for entering dates and times.  Allows user to pick date from popup TRzCalendar.
  TRzMemo               Enhanced TMemo--supports Custom Framing, Line & Column properties and events.
  TRzRichEdit           Enhanced TRichEdit--supports Custom Framing, Line & Column properties and events.
  TRzHotKeyEdit         Enhanced THotKey--supports Custom Framing.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Added OnGetWeekNumber event to TRzDateTimeEdit. Handle this event to implement a customized
                          week numbering scheme.
                        * Added OnRangeError event to TRzNumericEdit. This event is generated when the user leaves the
                          field and the value entered by the user exceeds the bounds defined by the Min and Max
                          properties.
                        * Fixed problem where Line property in TRzMemo and TRzRichEdit would not return the correct
                          value when text was selected.

  3.0.8  (29 Aug 2003)  * Fixed problem where user could not enter a positive number into a TRzNumericEdit if the
                          first character in the selection was a '-' sign.
                        * Fixed problem where csCaptureMouse ControlStyle was causing problems if the edit was not
                          allowed to get the focus via some user defined OnExit event handler for some other control.
                        * Fixed problem where TRzRichEdit would not honor DisabledColor value when disabled.
                        * Moved FFrameVisible instance field to protected section for access in TRzDBEdit.
                        * Fixed problem where user could still display the calendar or time picker when the
                          TRzDateTimeEdit.ReadOnly property was set to True.
                        * TRzDateTimeEdit no longer generates the OnExit event when the calendar or time picker are
                          displayed.
                        * Added OnDateTimeChange to TRzDateTimeEdit. This event fires whenever the date/time value in
                          the edit field changes.
                        * Fixed problem where selected text at beginning of a TRzNumericEdit that contained a '+' or '-'
                          sign could not be replaced by pressing the '+' or '-' keys.
                        * Fixed problem where OnChange event of TRzNumericEdit would fire during program load under
                          special circumstannce--e.g. current regional settings are different than those used by
                          developer.

  3.0.6  (11 Apr 2003)  * Fixed problem where using the TimePicker to set the time in a TRzDateTimeEdit's Time to
                          12:00 AM did not enter 12:00 AM into the edit field.
                        * Fixed problem where a digit could be entered to the left of a sign symbol in TRzNumericEdit.
                        * Modified TRzCustomEdit.KeyDown to prevent the vk_Down key from being further processed by
                          descendant components after the DoDropDown virtual method has been invoked. This change fixed
                          the problem where using Alt+Down to display the TimePicker for a TRzDateTimeEdit and setting
                          the desired time pressing Enter would not always set the time correctly.

  3.0.5  (24 Mar 2003)  * Fixed problem with enter 12:00 am into TRzDateTimeEdit and leaving the field changed the time
                          to 12:00 PM.
                        * Clear method now correctly clears the TRzDateTimeEdit and it internal datetime value.
                        * Modified TRzDBNumericEdit.EvaluteText so that string conversion is avoided if text just
                          contains a leading minus sign or open paren indicating a negative number. This change prevents
                          the beep from sounded if the text value cannot be converted.
                        * Fixed problem where leaving a TRzDateTimeEdit (with EditType=etTime) after deleting selected
                          time value caused 12:00 AM (or appropriately formatted time) to appear in control.

  3.0.4  (04 Mar 2003)  * Fixed problem where format of TRzDateTimeEdit would change when control received the focus.
                        * Fixed problem where focus would leave TRzDateTimeEdit if accelerator pressed and the key
                          was a valid date or time value key (e.g. 1, 2, 3...).
                        * Added OnCloseUp event to go with OnDropDown event.

  3.0.3  (21 Jan 2003)  * Fixed problem where clicking on TRzColorEdit in Delphi 5 caused the IDE to place control into
                          "move" mode.
                        * Added IsColorStored and IsFocusColorStored methods so that if control is disabled at design-
                          time the Color and FocusColor properties are not streamed with the disabled color value.
                        * Added additional hot keys to TRzDateTimeEdit for changing Day, Month, Year, and Hour, Minute.
                        * Fixed problem where OnKeyDown event was not getting generated.

  3.0    (20 Dec 2002)  << TRzCustomEdit and TRzEdit >>
                        * Simplified the TRzCustomEdit.CreateParams method. In previous versions, we had a check for
                          Win95 and NT 4, and under these conditions, the es_MultiLine style was used to support
                          right justification. However, this style introduced many other side-effects. Fortunately,
                          it is no longer necessary to include this style to get right-justification and therefore it
                          has been removed.
                        ...
                        * Fixed problem where the MaskEdit property editor was being used for editing the Text property of
                          the TRzEdit and descendants.

                        << TRzMaskEdit >>
                        * Added an OnValidateError event.  This event fires when a validation error occurs in edit mask.
                          This event allows a developer to specify a custom error message instead of the default
                          exception message defined in the VCL.

                        << TRzNumericEdit >>
                        * Added AllowBlank and BlankValue properties. Setting AllowBlank to True prevents the control from
                          automatically ensuring that the value is always a non-blank valid numeric value. This property
                          is most useful when connecting the data-aware version to a field that can have NULL values.
...
begin
  Result := SelStart - SendMessage( Handle, em_LineIndex, Line - 1, 0 ) + 1;
end;
...
begin
  ...
  Result := TopLine + ( P.Y div Canvas.TextHeight( 'Yy' ) ) + 1;
end;
