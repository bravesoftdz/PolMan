{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzGroupBar           Container control that manages one or more TRzGroup instance.  Groups are displayed in an XP
                          category view style, or in an Outlook navigator bar style.
  TRzGroup              Container control used within a TRzGroupBar to hold other controls or TRzGroupItems collection.
  TRzGroupItems         Collection class to hold TRzGroupItem instances.
  TRzGroupItem          Represents an entry on a TRzGroup--entry can be clicked to invoke OnClick event or an Action.

  TRzGroupTemplate      Nonvisual component used to define a template that can be used to create actual instances of
                          TRzGroup controls.
  ...
  TRzGroupController    Nonvisual component used to control the appearance of TRzGroup controls.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Added FontColor and FontStyle properties to TRzGroupTemplateItem.
                        * When creating a TRzGroup from a TRzGroupTemplate, the Tag property of the template is now
                          transferred to the group.
                        * Surfaced the PopupMenu property for TRzGroup.
                        * Added ItemPopupMenu property to TRzGoup. This menu is displayed when the user right-clicks an
                          item in the group.
                        * Added an OnItemPopupMenu event to TRzGroup. This event is raised just before the ItemPopupMenu
                          is displayed for an item. Use this event to make state changes or even assign differt menus to
                          ItemPopupMenu.
                        * Added PopupItem read-only public property to TRzGroup. This property references the
                          TRzGroupItem for which the popup menu was displayed. This property is usefully within
                          TMenuItem.OnClick or TAction.OnExecute event handlers to identify the group item.
                        * Removed the OnPopupClick event of TRzGroupItem. With the above changes, this event is no
                          longer necessary.
                        * Added ItemPopupMenu and PopupMenu properties to TRzGroupTemplate. Also added the
                          OnItemPopupMenu event.
                        * Fixed problem where separator items in a TRzGroup would get hidden inappropriately.

  3.0.8  (29 Aug 2003)  * Fixed problem where programmatically setting an item's Selected property to True did not cause
                          the corresponding group's ItemIndex property to be updated appropriately.
                        * System cursor (IDC_HAND) is now used instead of the custom hand cursor when running under
                          Windows 98 or higher.
                        * Added new GradientPath property to TRzGroupBar. This property controls the way the gradient
                          background of the group bar is drawn.
                        * Added ItemStaticFont property to TRzGroup. Use this property to specify a different font for
                          items in a group that do not have an OnClick event handler nor an assigned Action.
                        * Added FlatColorAdjustment property.
                        * Fixed problem where setting BorderInner or BorderOuter to fsFlatBold would not display border.
                        ...
                        * Fixed problem in WMNCCalcSize where border styles of fsFlatBold and the new fsFlatRounded
                          were not property considered in the size calculation.
                        * Added OnMouseOverItem event to the TRzGroup component. This event is triggered as the mouse is
                          moved over a linked item (i.e. OnClick or Action is assigned) in a TRzGroup.
                        * Fixed problem where dynamically created group needed to have parent set before adding to a
                          TRzGroupBar.
                        * Fixed problem where removing a group from a TRzGroupBar by calling RemoveGroup would leave
                          remants of the group on the group bar's display.
                        * TRzGroup will now automatically hide separators that are adjacent to other separators or
                          located at the top or bottom of the group.
                        * Added the TRzGroupTemplate component, and the supporting TRzGroupTemplateItems and
                          TRzGroupTemplateItem classes.  The TRzGroupTemplate component is used to define a template
                          (of items) that can be used to dynamically create actual instances of TRzGroup controls.
                        * The TRzGroupBar.AddGroup and TRzGroupBar.InsertGroup methods have been overloaded with
                          versions that accept a TRzGroupTemplate. These methods create a new instance of a TRzGroup and
                          populate the group with items matching those defined in the template.
                        * Added the TRzGroupController component. This nonvisual component can be used to manipulate the
                          appearance of multiple TRzGroup controls via the properties of the TRzGroupController
                          component.
                        * Added FontColor and FontStyle properties to TRzGroupItem.

  3.0.6  (11 Apr 2003)  * Fixed problem where a group with Style=gbsOutlook and more icons than can be displayed fully
                          would display the down scrolling button in the wrong position.
                        * Fixed problem where a PopupMenu assigned to the TRzGroupBar would appear when the user right-
                          clicked on a group item. In order to display a popup menu on a group item, the PopupMenu must
                          be invoked from within the OnPopupClick event of the item.
                        * Added ItemHotColor property.
                        * Added dt_RtlReading flag to DrawText function calls when UseRightToLeftAlignment = True.

  3.0.5  (24 Mar 2003)  * Added separation between items when TRzGroup.ItemStyle = isLarge to more clearly associate the
                          a caption to its icon.

  3.0.4  (04 Mar 2003)  * The TRzGroupBar now displays groups and items correctly (i.e. right aligned) when running
                          under Right-To-Left locales.
                        * Added public Data property to TRzGroup.
                        * Added mouse wheel support.
                        * When a group is opened and its opened height is greater than the height of the group bar, the
                          group is positioned so that the top of the group is in view.
                        * Fixed problem where scrollbar thumb would in some cases get positioned incorrectly when a
                          group was opened.
                        * Fixed problem where gradient background of group bar would get corrupted.
                        * Added ItemDestroyed notification method to TRzGroup in order to update ItemIndex property
                          when items are removed the group.
                        * Fixed problem where group items did not invoke standard action OnExecute events.
                        * In gbsCategoryView, groups now align child controls so that they do not cover the border of
                          the group.

  3.0.3  (21 Jan 2003)  * Test FullColorSupport before all calls to PaintGradient.
                        * Added code to remove & chars that cause underscores to appear in item captions.
                        * Fixed problem where border around large icons did not appear when color of group was clBtnFace.
                        ...
                        * Fixed problem where groups would not appear on Load when TRzGroupBar parented by panel.
...
type
  ...
  TRzGroupProperty = ( gpAll, gpBorderColor, gpCaptionColor, gpCaptionFont, gpCaptionHeight,
                       ...
                       gpSelectionColor, gpShowItemSelection, gpUseGradients );
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if ( ( SmallImageList = nil ) or ( FItems[ I ].ImageIndex = -1 ) ) and
