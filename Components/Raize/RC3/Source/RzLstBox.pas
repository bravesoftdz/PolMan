{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzListBox            Enhanced list box--adds custom framing, incremental keyboard searching, horizontal scroll bar.
  TRzTabbedListBox      TRzListBox descendant--uses Tab character to display items in columns.
  TRzEditListBox        Items in the list can be modified at runtime by the user.
  TRzRankListBox        Items can be rearranged in the list by dragging.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Fixed problem where tab stops were not being calculated correctly for a TRzTabbedListBox when
                          an item did not have any embedded tab (#9) characters and other items did.
                        * Modified TRzCustomListBox and descendants such that when the Items list is being updated
                          (i.e. within a BeginUpdate..EndUpdate block), AdjustHorzExtent and AdjustTabStops are not
                          called. The result of this is dramatically improved performance when loading files, etc into
                          the Items list.  After loading the file, call AdjustHorzExtent and/or AdjustTabStops to
                          instruct the control to make the necessary adjustments.
                        * Added IncrementalSearch property.
                        * Modified AjustHorzExtent method to take into account the OwnerDrawIndent value. This is
                          necessary for descendant controls such as the TRzCheckList that specify a non-zero return
                          value in OwnerDrawIndent to handle the display of glyphs (e.g. check boxes). 

  3.0.8  (29 Aug 2003)  * Moved group support from the TRzCheckList control into the base TRzCustomListBox control. This
                          includes the group-related properties (GroupColor, GroupFont, GroupPrefix, UseGradients,
                          ShowGroups), as well as all the group-related support methods.
                        * Fixed range error when using the Mouse Wheel and list box has more than 65,535 items.

  3.0.5  (24 Mar 2003)  * Fixed problem of item hints for TRzTabbedListBox show black boxes for tabs.

  3.0.4  (04 Mar 2003)  * When MultiSelect is True and ExtendedSelect is False, the space bar toggles the selection state
                          of the item instead of invoking the incremental searching function.

  3.0.3  (21 Jan 2003)  * Index of item changed in TRzEditListBox updated correctly if list is sorted.
                        * Add method override added to TRzTabStopList to handle updating tabstops after a tab stop is
                          added. Insert method is not called when Add it called.
  ...
  3.0    (20 Dec 2002)  << TRzCustomListBox and TRzListBox >>
                        * Added ShowItemHints property.  When set, a hint is displayed if the entire item cannot be
                          shown within the width of the list box.
                        ...
                        * Added the AdjustHorzExtent method, which is called automatically if the HorzScrollBar is set to
                          True.  This method automatically determines the appropriate horizontal extent needed to
                          completely display the longest string in the list.
                        * Added the OwnerDrawIndent property.  This property controls the rectangle used for each item
                          when the Style is in Owner-Draw Mode.  This property allows a user to have only a portion of
                          the item appear selected while the DrawItem event can draw anywhere in the original rectangle.
                        * Fixed problem with Delphi 7 version of List Box that prevents the user from changing the Color
                          of the list box.
                        * The Delete method has been enhanced so that if the index passed to the method is the same as
                          the item currently selected (i.e. ItemIndex), then the ItemIndex property will be updated to
                          select the adjacent item after the deletion.

                        << TRzEditListBox >>
                        * Fixed problem where pressing F2 when the list box was empty generated an Index out of Range
                          exception.
...
begin
  { Check to see if user presses Ctrl+C and stop it from triggering
    main menu Copy }
