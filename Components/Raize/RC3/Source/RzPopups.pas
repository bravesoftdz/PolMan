{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzCalendar           Calendar panel that displays dates in month view (Office-style)
  TRzTimePicker         Custom panel that displays a clock and allows a user to select a time using left and right
                          mouse buttons.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Fixed problem where TRzCalendar.OnDblClick would never get fired.
                        * Added OnGetWeekNumber event to TRzCalendar. Handle this event to implement a customized week
                          numbering scheme.
                        * Added Format property to TRzTimePicker. This property controls the format of the time that is
                          displayed at the top of the control.
                        * Set default values for TRzCalendar.CalendarColors and TRzTimePicker.ClockFaceColors.

  3.0.8  (29 Aug 2003)  * Made change to TRzPopup.Popup method so that when a popup window is displayed by a control,
                          the CPU utilization no longer hits 100%. 

  3.0.6  (11 Apr 2003)  * Fixed problem where StrToDateEx would try to interpret the year in a date specifying a month
                          name (e.g. April 11, 2003) as a month/year combintation without a date-separator. This lead
                          to the entire date being considered invalid, when in fact it was a valid date.

  3.0.4  (04 Mar 2003)  * Fixed problem where popups were not being displayed on the appropriate monitor when running
                          under a system supporting multiple monitors.

  3.0.3  (21 Jan 2003)  * Added keyboard support for TRzCalendar and TRzTimePicker.
                        * HowToUseHint is positioned at bottom of TRzTimePicker. Also, a timer is used to remove the hint
                          if the mouse is moved outside the clock face and the hint is still visible, which can happen
                          if the user moves the mouse quickly.
                        * Fixed display of Time header in TRzTimePicker so that it completely fills client region when
                          other border styles are used.
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ( L > 2 ) and ( L <= 6 ) and ( Pos( DateSeparator, S ) = 0 ) and ( Format <> 'Y' ) then
        begin
