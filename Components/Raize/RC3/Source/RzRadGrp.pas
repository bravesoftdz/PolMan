{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzRadioGroup         Mimics TRadioGroup but descends from TRzPanel and provides more display options.
  TRzCheckGroup         Similar to TRzRadioGroup, but creates embedded check boxes instead of radio buttons.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Added MouseDown event dispatch method override to TRzGroupButton in order to trap mouse
                          clicks if the control cannot be modified.  This is necessary when the radio group is a
                          TRzDBRadioGroup and the corresponding dataset is not in edit mode (or cannot be placed into
                          edit mode).
                        * Added the FrameController property to both the TRzRadioGroup and the TRzCheckGroup. If the
                          group has its GroupStyle property set to gsCustom, then the appearance of the group is
                          controlled by the settings of the TRzFrameController. In addition, if the radio buttons/check
                          boxes displayed within the group are set to HotTrack mode (i.e. flat), then the associated
                          FrameController determines the coloring of the radio button/check box frames.   

  3.0.8  (29 Aug 2003)  * Surfaced the OnPaint event in TRzRadioGroup and TRzCheckGroup. This is useful when GroupStyle
                          is set to gsCustom.

  3.0.4  (04 Mar 2003)  * Fixed display problem when under Right-To-Left locales.

  3.0    (20 Dec 2002)  << TRzCustomRadioGroup and TRzRadioGroup >>
                        * ItemFont only streamed when changed from the default component font.
                        * Added OnChanging event.  Developer can now prevent the user from changing the selected radio
                          button.
                        * Added a SpaceEvenly property. When set to True, the radio buttons contained in the group are
                          spaced evenly across the width of the group box.  This behavior is similar to the way in which
                          the standard TRadioGroup positions its buttons.
                        * Other controls can now be dropped onto a TRzRadioGroup.
                        * Added a FlatButtons property that controls the Flat properties of the embedded TRzRadioButtons.
                        * Radio button alignment automatically switched when run under Right-To-Left locales.
