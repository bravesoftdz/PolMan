{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  3.0.8  (29 Aug 2003)  * Surfaced Align property in TRzShellCombo.
                        * Fixed problem in TRzShellTree that would cause an AV when running under some versions of
                          ComCtl32 in Windows 98.

  3.0.4  (04 Mar 2003)  * Fixed flickering problem in TRzShellList caused by inherited FillLastColumn property.

  3.0.3  (21 Jan 2003)  * Fixed problem in TRzCustomShellTree.DoSetSelectedIdList method where selected node wasn't being
                          positioned correctly under certain circumstances.

  3.0    (20 Dec 2002)  * Initial inclusion in Raize Components.

  1.7a   --TBA--        * Added Wait cursor when Ole dropping onto TRzShellTree or TRzShellList controls.
                        * Fixed right-click menu on non-desktop BaseFolder nodes.
                        * Fixed various problems with renaming drives.
                        * Fixed 'drive not present' error when viewing removable drives, in TRzShellList, in vsReport
                          view, on Windows XP, when using themeing.
                        * Fixed problem when hovering over removable drives while drag-dropping into TRzShellTree. The
                          'please insert disk' message would pop-up, aborting the drag operation. Messages like that
                          are now ignored during drag-drop operations.
                        * Fixed TRzShellTree now re-sorts items after editing an item's name.

  1.7    (12 Sep 2002)  * Fixed memory leak in TRzShellTreeData.GetThisIShf.
                        * Fixed issue with usage of IContextMenu.GetCommandString on Windows XP which was causing problems
                          with some keyboard shortcuts (Ctrl+V, Ctrl+C, Del) and DoCommand* methods.
                        * Fixed refresh issue with TRzShellList icon view on Windows XP with themes.
                        * Fixed performance issue with TRzShellList on Windows XP when viewing folders containing large
                          .zip files.
                        * Fixed problem with TRzShellList where changing the control's Parent was exhibiting unexpected
                          behaviour.
                        * Reduced redundant refreshes on filesystem changes.

  1.6b   (07 Mar 2002)  * C++Builder 6 release.

  1.6a   (11 Jan 2002)  * Added hover processing for tree view. Now auto-expands items if you hover over them while
                          OLE dragging.
                        * Added AutoDropDownCount to TRzShellCombo.
                        * Fixed AV with DoCommandForFolder on some Windows installations.

  1.6    (02 Jul 2001)  * Added Delphi 6 support.
                        * Fixed popup menu for root-nodes. If the root node is not the Desktop, then the right click popup
                          menu is now correctly handled.
                        * Fixed 'access denied' error displayed on certain Windows 2000 systems due to the "System Volume
                          Information" folder.
                        * Fixed background color of state image list in TRzShellTree - now matches TRzShellTree.

  1.5c   (30 Mar 2001)  * Fixed AV which sometimes occured in TRzShellTree on Windows 98 when folders were created and
                          deleted.
                        * Fixed TRzShellListData so that new columns can be added.
                        * Changed to central acquisition of shell's IMalloc interface.

  1.5b   (12 Dec 2000)  * Fixed potential memory leak under Windows 2000.
                        * Fixed potential AV when performing multiple drag/drop operations with TRzShellTree.
                        * Fixed problem with image list. Certain types of images would show with a black border on some
                          Windows/IE/comctl combinations.
                        * Fixed refreshes of TRzShellTree could cause the currently selected node to jump up one or
                          two levels.

  1.5a   (14 May 2000)  * Added support for Windows 2000 - correct column information is now used for most non-filesystem
                          folders.
                        * Changed-removed support for CF_TEXT format in drag/drop operations. This more closely matches
                          Windows Explorer and prevents issues with some drop-target applications (such as Wordperfect).
                        * Fixed--Improved handling of case where a background refresh occurs, while a popup menu or edit
                          mode is active. Refreshes are deferred until the operation is complete.
                        * Fixed--Published RightClickSelect property in TRzShellTree.

  1.5    (03 Mar 2000)  * Added C++Builder 5 support.
                        * Improved performance of TRzShellCombo control by using new OnGetItemData method of
                          TRzImageCombo class.
                        * Fixed. Reversed the sort direction icon, now up=ascending, down=descending.
                        * Fixed. Published OnInfoTip event in TRzShellList.
                        * Fixed. TRzShellList.OnItemContextMenu and TRzShellTree.OnNodeContextMenu are now invoked
                          correctly.
                        * Fixed. TRzShellList Drag/drop now behaves more like Windows Explorer. Dropping on an item that
                          does not accept dropped files now 'drops-through' to the owning folder. The effect is like you
                          had dropped in empty white-space.
                        * Fixed. The update in v1.4a to better handle removable and dynamically mounted drives can cause
                          excessive floppy drive access on some systems. It seems to happen only on IE5 systems, but
                          only rarely. On those systems on which it does happen, it is reproducible. With this fix,
                          those systems will only access the floppy drive after a removable disk is changed, or a drive
                          is mounted/unmounted.

  1.4a   (15 Dec 1999)  * Fixed update and refresh of drive volume labels, dynamically mounted drives etc.
                        * Fixed problem where each new CDROM mounted would give a new tree node, without deleting the
                          previous node for that drive.
                        * Fixed case where folder column widths were sometimes not remembered between folders.
                        * Fixed problem when showing non-folders in the TRzShellTree controls by using the
                          stoIncludeNonFiles option. File list changes were not automatically detected, even when the
                          stoDynamicRefresh option was set. Manually refreshing the tree with F5 or by calling
                        * RefreshNodes also failed to reflect changes.
                        * Fixed OnContextPopup event in Delphi 5, now correctly published.
                        * Fixed OnInsertItem event now called for root node in TRzShellTree control.
                        * Added SFGAO_VALIDATE to some GetAttributes calls. Should help removeable disk changes be
                          handled better. Definately makes ZIP disks work better.

  1.4    (14 Sep 1999)  * Improved support for custom sorting using OnCompare and SortColumn:=0.
                        * Fixed OnCustomDraw, and OnCustomDrawItem. They are now published.
                        * Fixed editing of non-filesystem, renamable items like My Computer. It now works.
                        * Fixed OnGetImageIndex event of TRzShellList, it is now called correctly.
                        * Fixed problem where if a mapped network drive's server was shutdown, CPU utilization would go
                          to 100%.
                        * Fixed drag-drop error case where dropping from some data sources which don't support CF_IDLIST
                          format would cause an access violation.

  1.3h   (29 Mar 1999)  * Added support for Rename menu item in filesystem context menus.
                        * Changed RzShCreateNewFolder to support drives without long-filename capability.
                        * Improved robustness of TRzShellLocator ReadData and WriteData methods.
                        * Improved performance of some loops in TRzCustomShellList, now using GetNextItem instead of
                          iterating through the whole list.
                        * Fixed key handling of Shift/Delete. This key combo now deletes files immediately bypassing the
                          Recycle Bin in the same way as Windows Explorer.
                        * Fixed problem where mouse wheel was unresponsive for TRzShellList controls in vsList mode.
                        * Fixed OnDblClickOpen event in TRzShellList control. It was not being called when multiple items
                          were opened simultaneously with a double click or Enter key.

  1.3g   (01 Dec 1998)  * Added TRzCustomShellList.DoCommandForItem method.
                        * Added mouse wheel support to TRzShellTree and TRzShellList (Delphi 4 only).
                        * Added OnSelectItem event to TRzShellList (Delphi 4 only).
                        * Added support for more types of Explorer extensions (such as PicaView). Shell menu handling now
                          mimics better the behaviour of modern variants of Windows Explorer.
                        * Added OnFillStart event to TRzShellTree and TRzShellList.
                        * Changed OnFillComplete is now called at then end of Refresh methods as well as at the end of
                          FillItems methods.
                        * Changed TRzCustomShellTree.ShellSelCount. Now very much faster for large lists.
                        * Fixed bug in TRzCustomShellList.SetFileFilter method.
                        * Fixed problem where multiple filters would cause an empty floppy drive to timeout for each
                          filter. Now stops after the first timeout.

  1.3f   (12 Jul 1998)  * TRzCustomShellTree.SortNode moved from private to public section.
                        * Changed some parameter types to better support C++Builder 3.
                        * Changed some expressions and types to better support Delphi 4.

  1.3e   (22 Apr 1998)  * Fixed problem with TRzCustomShellList.RefreshItems not working correctly under Windows NT. Also
                          improved refresh algorithm making it another 2-3x faster.

  1.3d   (18 Apr 1998)  * Added OnCompare event to TRzShellList. If SortColumn is 0, then the OnCompare event is called
                          allowing definition of custom sort orders.
                        * Added OnFolderChanged event to TRzShellList. This event is fired after the Folder property
                          changes.
                        * Added OnInsertItem event to TRzShellTree. This event is called after a node has been added to
                          the tree.
                        * Changed algorithm used in TRzShellList.RefreshItems. It is now at least 100x faster on large
                          folders.
                        * Fixed Delphi drag support in TRzShellTree. If you disable OLE drag in TRzShellTree or
                          TRzShellList, you can then activate Delphi drag by setting the DragMode property to
                          dmAutomatic.
                        * Fixed problem with TRzShellList properties Checkboxes, GridLines, HotTrack and RowSelect not
                          being available at design time in C++Builder 3.

  1.3c   (16 Mar 1998)  * Added sloShowHidden and stoShowHidden options.
                        * Changed default autoscroll delay from 200ms to 100ms.
                        * Fixed memory leak in TRzShellList.RefreshItems.
                        * Fixed a number of cases where changing Options in TRzShellList and TRzShellTree controls at
                          run-time would have no effect.

  1.3b   (07 Feb 1998)  * Fixed: TRzShellListData.FDisplayName was not being cleared in the TRzShellListData.Flush method,
                          causing various, minor, strange behaviours at times, esp. when editing list item names.
                        * Fixed: When a tree and list were linked, and the tree control was recreated it would be empty.
                        * Fixed: When a tree with a non-null BaseFolder was linked to a list, the list would first show
                          the desktop, then the BaseFolder. It no longer initially shows the desktop.

  1.3a   (07 Jan 1998)  * Added extra validity checking for node and item .Data properties. This makes it possible to add
                          non-shell items to shell controls - see new NonShellNodes example.
                        * Changed OnEdited event. Now gets fired after the file/folder is renamed, rather than before.
                        * Fixed: Right-click on non-desktop base node would cause exception.
                        * Refixed: Fix for CreateNewFolder on empty BaseFolders in TRzShellTree didn't work.

  1.3    (28 Nov 1997)  * Added support in TRzShellList for remembering the widths of filesystem folder columns between
                          folder changes.
                        * Fixed non-functional OnExpanding event. Now gets fired.
                        * Fixed a problem on NT4 where setting a Filter in a TRzShellList would show an unnecessary error
                          box when attempting to view a removable disk when no disk was present.
                        * Fixed a problem under IE4 and Office97 where .htm, .doc, .xls files etc. could not be opened
                          with a double-click on the shell list control.
                        * Fixed a problem with TRzShellTree when BaseFolder was set to anything other than desktop, any
                          items dropped onto the root node would be dropped onto the desktop, rather than the root node.
                        * Fixed. When CreateNewFolder was called for TRzShellTree when the selected folder had no
                          sub-folders, then the "New Folder" node was not displayed or editable.
                        * Fixed non-functional stoDefaultKeyHandling and stoOleDrag Options in TRzShellTree. They now
                          work.
                        * Fixed TRzCustomShellCombo.GoUp and TRzCustomShellTree.GoUp methods. The were ignoring the
                          'levels' parameter.

  1.2b   (12 Oct 1997)  * OnFillComplete is now published in TRzShellList.
                        * Fixed a problem where changing the parent of a TRzShellCombo caused access violations. The
                          Windows combobox control was sending multiple WM_DELETEITEM messages per item. A work around
                          has been applied.
                        * Fixed IE4 problem where floppy was being redundantly accessed.
                        * Fixed IE4 problem with SendTo menu not working.
                        * Fixed GDI leak due to a problem with the placement of ShareImages:=True.

  1.2a   (05 Oct 1997)  * Added OnFillComplete event to TRzShellList and TRzShellTree.
                        * Fixed problem with accessing node data in OnInsert event.
                        * Fixed problem where after editing a list item's name, double clicking would use the previous
                          name.
                        * Fixed slowdown due to event race condition.
                        * TRzShellList option sloDontChangeFolder now works.
                        * TRzShellList.DoCommandForAllSelected no longer reports an "Unspecified Error" if there are no
                          items selected.

  1.2    (06 Sep 1997)  * C++Builder support, OLE drag-and-drop, SendTo improvements.
                        * Added sloOleDrag and sloOleDrop to the Options property of TRzShellList.
                        * Added stoOleDrag and stoOleDrop to the Options property of TRzShellTree.
                        * Added stoExpandWhenSelected to Options property of TRzShellTree.
                        * Moved icon loading to LVN_GETDISPINFO and TVN_GETDISPINFO notifications. This has the effect of
                          giving the tree and list views a progressive update - especially noticable on slow media such
                          as floppy or CDROM.
                        * Fixed problem where TRzShellList.OpenSelectedItems caused exceptions with large numbers of
                          items.
                        * Fixed DoCommand* methods. On some OS flavors not all DoCommand* messages worked. String verbs do
                          not always work as documented. Check the ExecuteContextMenuitem method for the workaround.
                        * Fixed SendTo - now uses a drag/drop approach that makes non .lnk/.pif files work. It even works
                          with Microsoft PowerToys now.
                        * Fixed bug with TRzTreeView.SelectedItem not returning correct item.
                        * Fixed memory leak and added further optimization in TRzListView.FillList .
                        * Fixed OnInsert event. You can now use the Data item (and the GetDataFromItem method) with the
                          passed TListItem.
                        * Optimized column click sorting. The list is no longer refilled on every column click.
                        * Replaced dummy-node mechanism with node.HasChildren.

  1.1    (26 Jun 1997)  * Delphi 3 support, various fixes and speed improvements.

  1.0    (21 Apr 1997)  * Released version 1.
...
begin
  ...
  begin
    idt := nil;
    try
      hr := GetDataFromNode( node ).GetIDropTarget( GetValidParentHWND( Self ), idt );
      if Succeeded( hr ) then
      begin
...
begin
  ...
  if SystemParametersInfo( SPI_GETMOUSEHOVERTIME, DWORD( @Default ), nil, 0 ) then
    RZSH_AUTOOPEN_DELAY_MS := Default;

  if SystemParametersInfo( SPI_GETMOUSEHOVERWIDTH, DWORD( @Default ), nil, 0 ) then
    RZSH_AUTOOPEN_THRESHOLD_X := Default;

  if SystemParametersInfo( SPI_GETMOUSEHOVERHEIGHT, DWORD( @Default ), nil, 0 ) then
