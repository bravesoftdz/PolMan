{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  TRzSplitter           Container component with two panes.  The size of each pane is controlled by the splitter bar.
  TRzSizePanel          Enhanced panel that can be resized at runtime using a sizing bar.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Fixed problem where panes of TRzSplitter would not pickup ParentColor changes.

  3.0.8  (29 Aug 2003)  * When RealTimeDrag is False, pressing the Escape key will now cancel the resizing operation
                          for both the TRzSplitter and the TRzSizePanel.
                        ...
                        * Added new ssGroupBar splitter bar style. This style is useful when the TRzSizePanel or a pane
                          of the TRzSplitter contains a TRzGroupBar in gbsCategoryView mode. The splitter bar is drawn
                          to match the background shading of the group bar. In order to support this, the following
                          properties were also added: GradientColorAdjustment, GradientPath, ThemeAware, and
                          UseGradients.

  3.0.4  (04 Mar 2003)  * Fixed problem where sizing bar was not displayed correctly when DoubleBuffered was set to True
                          in a TRzSizePanel.
                        * Fixed problem where nested splitters were causing form to be "modified" on load in IDE.

  3.0    (20 Dec 2002)  << TRzSplitter >>
                        * Fixed problem where fixed pane position was not restored properly when FixedPane was set to
                          fpLowerRight, the HotSpot was closed, and the splitter was resized.
                        * The HotSpot now works when the LockBar property is True. Therefore, the user can quickly close
                          a pane of the splitter but is not able to resize the pane.
                        * Fixed problem where the right and bottom sides of the splitter's outside border were not drawn
                          when DoubleBuffered was set to True.
                        * Fixed property where an Access Violation occurred in the splitter when ExpressBars components
                          (from Developer Express) were used on the same form as the splitter.
                        * The HotSpotDirection property can now be set to hsdBoth, which splits the HotSpot button into
                          2 areas: the upper-left area causes the splitter to go to the minimum side; the lower-right
                          area causes the splitter to go to the maximum side.

                        << TRzSizePanel >>
                        * The HotSpot now works when the LockBar property is True. Therefore, the user can quickly close
                          the size panel but is not able to resize it.
