{=======================================================================================================================
  ...
  TRzFieldStatus        Enhanced TRzStatusPane--adds ability to display a field label next to status caption.
  ...
  TRzKeyStatus          Enhanced TRzStatusPane--displays toggled state of CapsLock, NumLock, or ScrollLock keys.
  TRzResourceStatus     Enhanced TRzStatusPane--shows System resource usage under Win9X, shows Memory usage under WinNT.
  TRzMarqueeStatus      Enhanced TRzStatusPane--display Caption is scrolls across status pane.

  TRzVersionInfo        Nonvisual component that provides access to the Version Info block of an application.
  TRzVersionInfoStatus  Enhanced TRzFieldStatus--connects to a TRzVersionInfo component to display version information.


  Modification History
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Fixed AutoSize problem in TRzCustomFieldStatus.

  3.0.8  (29 Aug 2003)  * Added GetCustomKeyValue public method to TRzVersionInfo.
                        * Fixed problem where TRzClockStatus would not auto size if Format was changed.
                        * Surfaced Color property in TRzCustomStatusPane and all descendants. Also added the
                          ParentFillColor property, which when True causes the FillColor to match the ParentColor and
                          Color values.
                        * Frame of status pane now drawn correctly when FrameStyle set to fsFlatBold.
                        * Created new TRzFieldStatus component, which allows a field label to be displayed next to
                          status information.  This component now serves as the ancestor for the TRzVersionInfoStatus
                          and TRzDBStatusPane components.

  3.0.5  (24 Mar 2003)  * Added OnGetDateTime event to TRzClockStatus.

  3.0.4  (04 Mar 2003)  * Fixed problem where AutoSize wasn't calculating correct size when Caption contained an "&"

  3.0    (20 Dec 2002)  << TRzCustomStatusPane and TRzStatusPane >>
                        * A status pane will now properly resize itself if the font style of the status pane is changed.
                          For example, if the font style changes to fsBold, the size of the status pane will automatically
                          be updated to accommodate the entire text if the AutoSize property has been set to True.
                        * Added FlatColorAdjustment property.

                        << TRzGlyphStatus >>
                        * Added Images, ImageIndex, DisabledIndex properties to TRzGlyphStatus. Therefore, the status
                          component can display its glyph from either a bitmap file or from an image list.

                        << TRzMarqueeStatus >>
                        * Enhanced the animation effects so that scrolling of text is much smoother.

                        << TRzKeyStatus >>
                        * When CapsLock, ScrLock, or NumLock keys are off, no caption appears in the status pane.  Also
                          fixed problem where key text would appear even if Visible property was set to False.
                        * Added the tkInsert key type and the InsertState property. When Key is set to tkInsert the status
                          pane will show the Insert or Overwrite state depending on the initial value of InsertState
                          when the program starts and the number of times the Insert key is toggled.  The InsertState
                          property can be used to determine the current insert state (isInsert or isOverwrite) as well
                          as change the current intrpretation of the Insert key with respect to the status pane.

                        << TRzClockStatus >>
                        * Fixed problem where clock text still appeared even though the Visible property of the component
                          was set to False.

                        << TRzResourceStatus >>
                        * Removed the ResourceType property.  Under Windows 95/98/ME this control shows the percentage of
                          Free System resources.  Under Windows NT/2000/XP, this control shows the percentage of Free
                          Memory.
