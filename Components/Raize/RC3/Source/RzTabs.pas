{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  3.0.9  (22 Sep 2003)  * Fixed problem where making TabHeight or TabWidth > 0 would cause the tabs to disappear if the
                          page control less than a certain size.
                        * Fixed problem where if ActivePage and ActivePageDefault were different pages, then upon
                          loading, both pages would be considered visible by the page control.
                        * Fixed problem where all tabs would be re-colored to active page color when MultiLine = True.
                        * Fixed problem where clicking on secondary rows of tabs (MultiLine = True) would not bring the
                          clicked tab to the front at design-time.  

  3.0.8  (29 Aug 2003)  * Initialized FShowFocusRect in constructor.
                        * Modified GetScrollerSize and GetCloseButtonSize to take into account FixedDimension when
                          calculating the size of the buttons.
                        * Added AllowTabDragging property.  When set to True, the tabs can be reordered by dragging them
                          with the mouse.
                        * Fixed problem where OnChange event was being fired when the control was first loaded.
                        * Clearing a TRzTabControl's Tabs collection now immediately sets the TabIndex to -1.
                        * TRzPageControl can now be a dock site and as new clients are docked to the page control, new
                          tab sheets are created to hold the dock clients.
                        * Made Change event dispatch method public.
                        * Fixed problem where deleting a tab in a TRzTabControl that is not active would raize an
                          index out of range exception.
                        * Fixed problem where setting Form.ActiveControl to some control contained on a TRzTabSheet in a
                          TRzPageControl would not focus that control when the form was loaded.
                        * Fixed problem where focus rectangle would clip vertically oriented text.

  3.0.6  (11 Apr 2003)  * Fixed problem where Index Out Of Range error would occur if all pages were removed from a
                          TRzPageControl while inside a LockWindowUpdate call.
                        * Fixed problem where Close button would be visible if TabHeight was something other than zero.
                        * Fixed problem where ActivePageIndex property would cause an access violation if no page was
                          currently active.
                        * Fixed problem where TRzPageControl would move at design-time if user clicked on a new tab and
                          then clicked on another designer form such as the Object Inspector.
                        * Added Tag and Data properties to TRzTabCollectionItem.
                        * Added Data property to TRzTabSheet.
                        * Adjusted position of tab scroller buttons and close button (if visible) such that they take
                          into account the Margin property value.
                        * Fixed problem where scroller button enabled states were incorrect when orientation
                          was toRight.
                        * Added ShowFullFrame property to TRzPageControl. When this property is set to False, only the
                          portion of the frame that is adjacent to the tabs is drawn.
                        * Added OnResize event to TRzTabSheet.
                        * Fixed problem where Anchors were not being honored for controls that appeared on non-active
                          tabs in a TRzPageControl -and- the page control was resized before switching to the other
                          tabs.
                        * Fixed problem where programmatically changing ActivePage or TabIndex did not update enabled
                          states of scroller buttons.

  3.0.5  (24 Mar 2003)  * Further enhanced performance of controls when HotTracking was set to True.
                        ...
                        * Added CloseActiveTab method. This method is called when the Close button is clicked and
                          generates the OnClose event and takes care of destroying/deleting the active tab.
                        * Added GetDisplayName override to TRzTabCollectionItem.
                        * Fixed display states of the scrolling buttons.

  3.0.4  (04 Mar 2003)  * Fixed problem where tabs extended past edge of card if ShowShadow was True.
                        * Fixed problem where Ctrl+Tab caused blank page to appear (i.e. page control itself) if no tabs
                          are visible.
                        * Fixed performance problem that occurred when hot tracking and ShowShadow set to True.
                        * Fixed problem where FixedDimension was not getting initialized correctly. This caused a problem
                          when using Anchors for windowed controls inside the TRzPageControl--3.0.3 addressed graphic
                          controls. For 3.0.4, the FixedDimension value needed to be stored to the stream in a
  ...
  3.0.3  (21 Jan 2003)  * Test FullColorSupport before all calls to PaintGradient.
                        ...
                        * Updated Scroller buttons to be displayed when first and/or last tab is visible.
                        * Fixed problem where FixedDimension was not getting initialized correctly. This caused a problem
                          when using Anchors for graphic controls inside the TRzPageControl.
...
var
  ...
  (*
  procedure GenerateContinuousXPCommands;
  begin
    ...
    AddCommandPt( cmd_MoveTo, Pts[ 1 ] );
  end; {= GenerateContinuousXPCommands =}
  *)

  procedure GenerateBrokenXPCommands;
  begin
...
begin {= TRzCustomTabControl.CalcCardDrawCommands =}
  ...
  begin
    ...
    case FTabStyle of
      ...
      begin
        if ( ARow > 0 ) or not CurrentTabInView then // Continuous card border
          //GenerateContinuousXPCommands
          GenerateContinuousBorderCommands
        else
          GenerateBrokenXPCommands;
      end;
    end;
  end;
