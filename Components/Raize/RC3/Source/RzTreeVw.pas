{=======================================================================================================================
  ...
  ----------------------------------------------------------------------------------------------------------------------
  3.0.8  (29 Aug 2003)  * Added HighlightColor to TRzCheckTree to allow user to change check mark color.

  3.0.5  (24 Mar 2003)  * Fixed problem where FullExpand and FullCollapse did not work correctly when AutoSelect=True.
                        * Fixed problem where correct node was not selected when RightClickSelect was set to True and
                          user right-clicked a different node from the currently selected node.
                        * Fixed problem where PopupMenu would not be displayed unless user right-clicked on a node.
  ...
  3.0.3  (21 Jan 2003)  * Added IsColorStored and IsFocusColorStored methods so that if control is disabled at design-time
                        ...
                        * Enhanced TRzCheckTreeStrings.SaveTreeToStream and TRzCheckTreeStrings.LoadTreeFromStream methods
                          so that the ImageIndex and SelectedIndex in addition to the StateIndex for each node is also
                          saved. The index values can also be more than one digit long.

  3.0    (20 Dec 2002)  << TRzCustomTreeView and TRzTreeView >>
                        ...
                        * Added the following new method:
                          ...
                          This method is used after moving a node with the MoveTo method. The standard TTreeView component
                          does not correctly update the display of a node that specifies a StateIndex.  More precisely,
                          the state image associated with the index is displayed next to the node after it has been
                          moved.  Call this method to ensure that the state image of the node (and all of its children)
                          are displayed.
                        ...
                        * Added the following new methods:
                          ...
                          These methods are used after moving a node in a TRzCheckTree with the MoveTo method. These two
                          methods need to be called after the move to ensure that the check states of the other nodes in
                          the tree remain in sync.  Please note that these methods are only needed when CascadeChecks is
                          True.

                          Before moving a node in a TRzCheckTree, save the ParentNode of the node to be moved. After
                          performing the move (and calling UpdateStateIndexDisplay--see above), then make the following
                          calls:
                        ...
                        * Surfaced StateImages property.  This allows a user to provide custom images to be used for the
                          check states.

                        * Added the following methods: LoadFromFile, LoadFromStream, SaveToFile, SaveToStream.  These
                          methods allow you to persist not only the items and their hierarchical relations, but also the
                          states of each node in the tree.
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          if UpperCase( PathFromNode( SearchNode ) ) = FindPath then
          begin
